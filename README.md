...
**EN.** Every five minutes a scheduled task on the authors' own PC runs `tools/collect_daemon.py tick`. It reads `research/COLLECTORS.json`, and for every source that is due it first checks the permission ledger (`research/08-provenance/LEDGER.jsonl` — the source's own robots.txt, headers and licence page, stored with hashes). Only a permitted source is fetched, through verified TLS. The request headers actually sent are preserved in the evidence, and named refusals are honoured. Every value is stored with three times — measured (if the source says), published, received — a missing value is `null`, never zero; measurements are never filled in or replaced by forecasts. Receipts are never overwritten. Other scheduled tasks run the model organs, watch liveness, rebuild baseline/context layers, guard permission invariants and publish the filtered mirror. The rows feed studies (`research/05-design/studies/traka-live.html`, `monolog-puls.html`) and organs — automatically selected reviewed CLI and local models registered in `research/ORGANS.json`, whose every sentence is marked as model-derived. Rules: [CONTRIBUTING.md](CONTRIBUTING.md). Proof of permission: [research/08-provenance/INDEX.md](research/08-provenance/INDEX.md). Situation: [research/OPERATIONAL_ORDER_2026-09-11.md](research/OPERATIONAL_ORDER_2026-09-11.md).

**SR.** Svakih pet minuta zakazani zadatak na jednom računaru autora pokreće `tools/collect_daemon.py tick`. On čita `research/COLLECTORS.json` i za svaki izvor koji je na redu prvo proveri knjigu dozvola (`research/08-provenance/LEDGER.jsonl` — robots.txt, zaglavlja i stranica licence samog izvora, sačuvani sa hešovima). Samo dozvoljen izvor se čita, kroz proveren TLS. Stvarno poslata zaglavlja zahteva čuvaju se u evidenciji, a imenovana odbijanja se poštuju. Svaka vrednost se čuva sa tri vremena — izmereno (ako izvor kaže), objavljeno, primljeno — nedostajuća vrednost je `null`, nikad nula; merenja se ne popunjavaju niti zamenjuju prognozama. Priznanice se nikad ne prepisuju. Drugi zakazani zadaci pokreću organe modela, čuvaju ritam, grade osnovne slojeve, proveravaju dozvole i objavljuju filtrirani javni mirror. Redovi hrane studije (`research/05-design/studies/traka-live.html`, `monolog-puls.html`) i organe — automatski izabrane pregledane CLI i lokalne modele upisane u `research/ORGANS.json`, čija je svaka rečenica označena kao izvedena modelom. Pravila: [CONTRIBUTING.md](CONTRIBUTING.md). Dokaz dozvole: [research/08-provenance/INDEX.md](research/08-provenance/INDEX.md).

The citizen calendar also reads Kolarac's official repertoire (S225) through `tools/collect_events.py --refresh`, at most once per six hours. Upcoming terms are forecasts from one official source; keyword-classified headline signals stay untimed and unverified. A repertoire cache older than 24 hours is not presented as current. End times and locations not stated by the source remain unknown; this is not a complete city calendar or independent corroboration.
...
Current paper direction: pre-paper v5 (private working record), with the statistical collaboration note (private working record), [stability review](research/STABILITY_REVIEW_2026-09-10.md) and [decision ledger](research/DECISIONS.md). It supersedes the Serbian V2 discussion pack, v3/v4 drafts and their addenda as the current discussion entry. The older Serbian pre-paper V2 (private working record), earlier English pack (private working record) and all189 source atlas (private working record) are preserved history, not synchronized current texts.
...
Earlier situation review: 2026-09-05 afternoon (private working record) — repository/pilot defects, conference dates and the work then remaining.
...
Conference: Late-submission enquiry, drafted and not sent (private working record). The call's own Important Dates are 01.05.2026 for abstracts and **20.08.2026 for full manuscripts**; the conference is on-line on 25.09.2026. The manuscript deadline has passed and no acceptance is assumed.


Latest detailed program: Senses, specialist models and useful city pulses (private working record), with 64 search seeds (private working record) and [the expanded model/provider audit](research/03-models/KATALOG_MODELA_I_PROVAJDERA_2026-09-05.md). It elaborates the existing work program; it is not a claim that all experiments have run.
...
- Zones and external linking (private working record).
...
- [Source registry](research/SOURCE_REGISTRY.json): 222 source records, including archives, catalogues, related routes and coverage gaps. This is not a count of devices or live feeds.
...
- Pazarac transfer audit (private working record): read-only code findings and transfer boundaries.
...
3. Define layered city pulses and controlled comparisons in Research Protocol (private working record).
