# Registers (or replaces) the five BEOPS scheduled tasks. S4U logon so they run whether or not a
# user is signed in (an Interactive-only task silently skips and leaves the previous Last Result).
# Verify afterwards with: schtasks /query /tn Beops_Collect /tn Beops_Organ /tn Beops_Publish /tn Beops_Mind /tn Beops_Legal /v /fo list
...
$tasks = @(
  ...
  @{ name='Beops_Watch';   bat='tools\watch_tick.bat';   minutes=10; limit=5;  desc='BEOPS: the watchman - reads artefacts, never a task status' },
