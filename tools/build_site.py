...
.herohead{display:grid;grid-template-columns:minmax(0,1fr) minmax(480px,1.3fr);gap:calc(var(--u)*6) calc(var(--u)*8);align-items:start;margin-bottom:calc(var(--u)*6)}
.herocopy{display:flex;flex-direction:column;gap:calc(var(--u)*3.5)}
...
.hero h1{font-size:clamp(23px,2.4vw,33px);line-height:1.12;margin:0;font-weight:600;letter-spacing:-.015em;max-width:none;text-wrap:balance}
.hero h1 em{font-style:normal;color:var(--signal)}
.hero .lede2{font-size:clamp(14px,1.1vw,15.5px);color:var(--ink70);max-width:none;margin:0;line-height:1.55}
.hero .sub{font-size:clamp(13.5px,1.05vw,15px);color:var(--ink70);max-width:none;margin:0;line-height:1.55}
...
<header>
  <div class="wrap hbar">
    <a href="index.html" class="brand" style="margin-right:12px;font-size:13px;border:1px solid var(--ink30);padding:3px 8px;border-radius:3px">← Beograd danas</a>
...
<nav>
    ...
    <button type="button" id="theme" class="themebtn" title="Svetlo / tamno · Light / dark" aria-label="Svetlo / tamno · Light / dark">◐</button><div class="hlangs" id="lang" role="group" aria-label="Jezik / Language"><button type="button" data-l="sr" aria-pressed="true">SR</button><button type="button" data-l="en" aria-pressed="false">EN</button></div>
  </div>
</header>
  ...
  <div class="wrap">
    <div class="whathead">
      ...
      <div class="wlangs" role="group" aria-label="Jezik / Language">
        <button type="button" data-w="sr" aria-pressed="true">SR</button><button type="button" data-w="en" aria-pressed="false">EN</button>
      </div>
    </div>
    <div class="whatgrid">
      <div>
      ...
      <div>
        ...
        <p><span class="w-sr">Pošten odgovor je: pojedinačno nisu obavešteni. Poštuju se pravila koja su sami javno objavili — robots.txt, uslovi korišćenja, Content-Signal — i ta pravila se sačuvaju kao bajtovi pre svakog čitanja. Stvarno poslata zaglavlja zahteva čuvaju se u evidenciji. Imenovano odbijanje se poštuje i ne zaobilazi, a provera se ponavlja jednom nedeljno. Ništa se ne uzima iza prijave, iza plaćanja ni iza zabrane. Ako objavljujete neki od ovih izvora i ne želite da Vas čitamo, dovoljna je jedna poruka na <a href="mailto:poturaksemir@gmail.com">poturaksemir@gmail.com</a> — prestajemo u roku od 24 sata, a u registru ostaje zapisano ko je to tražio i kada.</span><span class="w-en">The honest answer is: they have not been individually notified. What is honoured are the rules they themselves published — robots.txt, terms of use, Content-Signal — and those rules are stored as bytes before any reading. The request headers actually sent are preserved in the evidence. A named refusal is honoured and never circumvented, and the check is repeated weekly. Nothing is taken from behind a login, a paywall or a prohibition. If you publish one of these sources and would rather we did not read it, one message to <a href="mailto:poturaksemir@gmail.com">poturaksemir@gmail.com</a> is enough — we stop within 24 hours, and the register records who asked and when.</span><span class="w-zh">诚实的回答是：没有逐一通知它们。我们遵守的是它们自己公开发布的规则——robots.txt、使用条款、Content-Signal——并在每次读取之前将这些规则以字节形式存档。实际发送的请求头保存在证据记录中。明确指名的拒绝会得到遵守，绝不规避；该检查每周重复一次。不从登录、付费墙或禁止访问的位置获取任何内容。如果您是其中某个来源的发布方，且不希望我们读取，只需发一封邮件至 <a href="mailto:poturaksemir@gmail.com">poturaksemir@gmail.com</a>——我们将在 24 小时内停止，并在登记册中记录是谁提出的以及提出的时间。</span><span class="w-de">Die ehrliche Antwort lautet: einzeln benachrichtigt wurden sie nicht. Beachtet werden die Regeln, die sie selbst veröffentlicht haben — robots.txt, Nutzungsbedingungen, Content-Signal — und diese Regeln werden vor jedem Lesen als Bytes gespeichert. Die tatsächlich gesendeten Anfrage-Header bleiben in den Belegen erhalten. Eine namentliche Ablehnung wird befolgt und nie umgangen; die Prüfung wird wöchentlich wiederholt. Nichts wird hinter einem Login, einer Bezahlschranke oder einem Verbot geholt. Wenn Sie eine dieser Quellen veröffentlichen und nicht möchten, dass wir sie lesen, genügt eine Nachricht an <a href="mailto:poturaksemir@gmail.com">poturaksemir@gmail.com</a> — wir hören innerhalb von 24 Stunden auf, und im Register bleibt festgehalten, wer darum gebeten hat und wann.</span></p>
      </div>
      <div>
  ...
  <div class="wrap">
    ...
    <div class="cols">
      <div>
        ...
        <p><span class="sr-only i18n">Pravila pristupa, zaglavlja i raspoloživi uslovi korišćenja čuvaju se kao bajtovi sa hešom pre prikupljanja. Nepoznato nikada nije dozvola. Evidencija čuva stvarno poslata zaglavlja zahteva. Imenovana odbijanja se poštuju i beleže da se zabranjeno prikupljanje ne ponovi.</span><span class="en-only i18n">Access rules, headers and available terms of use are stored as hashed bytes before collection. An unknown is never a permission. The evidence preserves the request headers actually sent. Named refusals are honoured and recorded to prevent prohibited collection from recurring.</span><span class="zh-only">采集前，访问规则、响应头及可获得的使用条款会以带哈希的原始字节保存。未知从来不等于许可。证据记录保存实际发送的请求头。明确指名的拒绝会得到遵守并被记录，以防再次进行被禁止的采集。</span><span class="de-only">Zugriffsregeln, Header und verfügbare Nutzungsbedingungen werden vor der Erhebung als Bytes mit Prüfsumme gespeichert. Unbekannt bedeutet niemals erlaubt. Die Belege bewahren die tatsächlich gesendeten Anfrage-Header. Namentliche Ablehnungen werden befolgt und dokumentiert, damit eine untersagte Erhebung nicht wiederholt wird.</span></p>
      ...
      <div>
        ...
        <h3 style="margin-top:20px"><span class="sr-only i18n">Organi — provereni modeli</span><span class="en-only i18n">Organs — reviewed models</span><span class="zh-only">器官——经过审查的模型</span><span class="de-only">Organe — geprüfte Modelle</span></h3>
        <div id="organs"></div>
      </div>
    </div>
  </div>
...
<footer>
  <div class="wrap fgrid">
    ...
    <div><b><span class="sr-only i18n">Gde ovo stoji</span><span class="en-only i18n">Where this runs</span><span class="zh-only">它运行在哪里</span><span class="de-only">Wo das läuft</span></b><span class="sr-only i18n">Prikupljanje i ceo zapis rade na sopstvenom računaru autora, u Srbiji. Objavljena stranica i otvoreni kod stoje na GitHub-u (GitHub Pages) — dakle na serverima van Srbije. Sačuvani dokazi o dozvolama i cele tuđe stranice nikada ne napuštaju taj računar. Um automatski bira pregledane modele preko lokalnog Codex CLI mosta; udaljenom modelu šalje samo zamrznut sažetak javnih činjenica, bez alata i pristupa fajlovima. Eksperimentalna „Zapažanja“ automatski biraju pregledanog Gemini ili ChatGPT provajdera i šalju najviše osam novih vrednosti sa dozvoljenih mernih izvora i objavljenih statističkih tabela — bez članaka i dokaza o dozvolama. Stvarni model je naveden uz svaki rezultat.</span><span class="en-only i18n">Collection and the complete record run on the authors' own computer in Serbia. The published page and open code are on GitHub Pages, on servers outside Serbia. Stored permission evidence and complete third-party pages never leave that computer. The mind automatically selects reviewed models through a local Codex CLI bridge; a remote model receives only a frozen digest of public facts, without tools or file access. The experimental “AI observations” automatically select a reviewed Gemini or ChatGPT provider and send at most eight recent values from permitted measurement sources and published statistical tables — no articles or permission evidence. The actual model is named with every result.</span><span class="zh-only">采集与完整记录运行在作者位于塞尔维亚的自有计算机上。已发布页面与开放代码位于境外的 GitHub Pages。保存的许可证据与第三方完整页面从不离开该计算机。心智通过本地 Codex CLI 桥自动选择经过审查的模型；远程模型只接收冻结的公开事实摘要，不具备工具或文件访问权限。实验性的“AI 观察”会自动选择经过审查的 Gemini 或 ChatGPT 提供方，并仅发送最多八个来自获准测量来源和已发布统计表的最新数值——不发送文章或许可证据。每项结果都会标明实际使用的模型。</span><span class="de-only">Erhebung und vollständiger Datenbestand laufen auf dem eigenen Rechner der Autoren in Serbien. Die veröffentlichte Seite und der offene Code liegen auf GitHub Pages, also auf Servern außerhalb Serbiens. Gespeicherte Erlaubnisnachweise und vollständige fremde Seiten verlassen diesen Rechner nie. Der Geist wählt geprüfte Modelle automatisch über eine lokale Codex-CLI-Brücke; ein entferntes Modell erhält nur einen eingefrorenen Auszug öffentlicher Fakten, ohne Werkzeuge oder Dateizugriff. Die experimentellen „KI-Beobachtungen“ wählen automatisch einen geprüften Gemini- oder ChatGPT-Anbieter und senden höchstens acht aktuelle Werte erlaubter Messquellen und veröffentlichter Statistiktabellen — keine Artikel oder Erlaubnisnachweise. Das tatsächlich verwendete Modell steht bei jedem Ergebnis.</span></div>
...
def main() -> int:
    ...
    for src, dst in [("research/05-design/studies/accessibility.css", "accessibility.css"),
                     ...
                     ("public/events.json", "events.json"),
                     ("public/mapa.html", "mapa.html"),
                     ("public/mapa.js", "mapa.js"),
                     ("research/MAP_LAYERS.json", "MAP_LAYERS.json"),
                     ("public/materija.json", "materija.json")]:
        p = ROOT / src
        if p.exists():
