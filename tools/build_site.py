...
<footer>
  <div class="wrap fgrid">
    ...
    <div><b><span class="sr-only i18n">Gde ovo stoji</span><span class="en-only i18n">Where this runs</span><span class="zh-only">它运行在哪里</span><span class="de-only">Wo das läuft</span></b><span class="sr-only i18n">Prikupljanje i ceo zapis rade na sopstvenom računaru autora, u Srbiji. Objavljena stranica i otvoreni kod stoje na GitHub-u (GitHub Pages) — dakle na serverima van Srbije. Sačuvani dokazi o dozvolama, tuđe stranice, nikada ne napuštaju taj računar. Lokalni modeli uma rade na istom računaru. Eksperimentalni panel „Zapažanja“ piše Google Gemini, van Srbije, iz najviše osam novih vrednosti sa dozvoljenih mernih izvora i objavljenih statističkih tabela — bez naslova, tekstova i dokaza o dozvolama. Svi modeli su navedeni u odluci D-003.</span><span class="en-only i18n">The collection and the whole record run on the authors' own computer, in Serbia. The published page and the open code are on GitHub (GitHub Pages) — that is, on servers outside Serbia. The stored permission evidence, which is other people's pages, never leaves that computer. The mind's local models run on the same computer. The experimental “AI observations” panel is written by Google Gemini, outside Serbia, from at most eight recent values of permitted measurement sources and published statistical tables — no headlines, articles or permission evidence. Every model is named in decision D-003.</span><span class="zh-only">采集与全部记录运行在作者自己的计算机上，位于塞尔维亚。已发布的页面与开源代码托管在 GitHub（GitHub Pages）上，也就是塞尔维亚境外的服务器。保存下来的许可证据——他人的页面——从不离开那台计算机。心智的本地模型在同一台计算机上运行。实验性的“AI 观察”面板由塞尔维亚境外的 Google Gemini 撰写，依据最多八个来自许可测量来源的最新数值和已发布的统计表——不含标题、文章或许可证据。所有模型均在决定 D-003 中列明。</span><span class="de-only">Die Erhebung und der gesamte Datenbestand laufen auf dem eigenen Rechner der Autoren, in Serbien. Die veröffentlichte Seite und der offene Code liegen auf GitHub (GitHub Pages) — also auf Servern außerhalb Serbiens. Die gespeicherten Erlaubnisnachweise, fremde Seiten, verlassen diesen Rechner nie. Die lokalen Modelle des Geistes laufen auf demselben Rechner. Das experimentelle Feld „KI-Beobachtungen“ schreibt Google Gemini außerhalb Serbiens aus höchstens acht aktuellen Werten erlaubter Messquellen und veröffentlichten Statistiktabellen – ohne Schlagzeilen, Artikel oder Erlaubnisnachweise. Alle Modelle sind in der Entscheidung D-003 genannt.</span></div>
