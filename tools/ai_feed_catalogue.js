...
function selectModel(rows, provider, state={}, now=new Date(), isLocal=()=>true) {
  ...
  const configured=Array.isArray(provider.models)&&provider.models.length
    ?new Map(provider.models.map((model,index)=>[model,index])):null;
  for(const row of rows) {
    if(row.prov!=='cli'||row.bridge!==provider.catalogue_bridge||!isLocal(row.device))continue;
    if(configured&&!configured.has(row.model))continue;
    ...
    eligible.push({...row,last_at:previous?.at||null,configured_order:configured?.get(row.model)??null});
  }
  eligible.sort((a,b)=>(a.last_at?Date.parse(a.last_at):0)-(b.last_at?Date.parse(b.last_at):0) ||
    (a.configured_order??Number.MAX_SAFE_INTEGER)-(b.configured_order??Number.MAX_SAFE_INTEGER) ||
    (a.costTier||2)-(b.costTier||2) || (b.speedTier||1)-(a.speedTier||1) || a.id.localeCompare(b.id));
